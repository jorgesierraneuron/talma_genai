...
class ChainManuales: 
    ...
    timeout=None,
    max_retries=2,
    api_key="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"
    )
...
class VisionRAG: 
    ...
    openai_client = OpenAI(api_key="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")

    chain_manuales = ChainManuales()

    qdrant_client = QdrantClient(
    url="https://481d5f37-ad78-47b9-9fc6-b4d7f67bcd36.us-east4-0.gcp.cloud.qdrant.io:6333",
    api_key="aJrKg4iIZr6Bg0WqzUji4xd7YWgXHIWq_nEMGddzkudpNdf4evbixQ",
    )
    ...
    colpali_model = ColPali.from_pretrained("/opt/colpali_model")
    colpali_processor = ColPaliProcessor.from_pretrained("/opt/colpali_processor")
    ...
    def __search_qdrant(self,query, collection_name, manual_name): 
        ...
        #             match=models.MatchValue(value="M-MTO-001 MANUAL GENERAL DE MANTENIMIENTO SAI REV.13"),
        #         )
        #     ]
        # )
        ...
        manual_name = search_result.points[0].payload["NOMBRE DOCUMENTO"]
        idx = search_result.points[0].id
        return idx, manual_name
    ...
    def run(self, query): 
        ...
        query_manual, manual_name_2 = self.__get_manual_name_query_manuals(query)

        idx, manual_name = self.__search_qdrant(query_manual, "manuales_talma_dev",manual_name_2)
        
        print(f"Manual name 2: {manual_name_2}")
...
#     print(vision_rag.run(descripcion_evento))
